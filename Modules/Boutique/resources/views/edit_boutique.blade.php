...
<main class="flex-1 p-4 md:p-6">
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
    ...
    <!-- النموذج -->
  <form  
    class="bg-white border border-pink-100 shadow-md rounded-2xl p-6 sm:p-8 space-y-8" 
    id="add_boutique">
    @csrf

    <input type="hidden" name="boutique_id" id="boutique_id" value="{{ $boutique->id ?? '' }}">
    ...
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div>
                ...
                <select name="status"
                        class="form-input w-full h-11 mt-1 rounded-xl border border-pink-200 focus:ring-2 focus:ring-[var(--primary-color)] px-3">
                    <option value="1" {{ isset($boutique) && $boutique->status == 1 ? 'selected' : '' }}>
                        {{ trans('messages.active', [], session('locale')) }}
                    </option>
                    <option value="2" {{ isset($boutique) && $boutique->status == 2 ? 'selected' : '' }}>
    ...
    <div>
        ...
        <textarea name="boutique_address" rows="3" id="boutique_address"
                  class="form-input w-full rounded-xl border border-pink-200 focus:ring-2 focus:ring-[var(--primary-color)] px-3"
                  placeholder="{{ trans('messages.boutique_address_placeholder', [], session('locale')) }}">{{ $boutique->boutique_address ?? '' }}</textarea>
    </div>
