...
@section('main')
@push('title')
<title>{{ trans('messages.area_lang', [], session('locale')) }}</title>
@endpush
...
<main class="flex-1 p-8 bg-background-light dark:bg-background-dark overflow-y-auto" 
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
            <h2 class="text-2xl sm:text-4xl font-bold text-[var(--text-primary)]">
                {{ trans('messages.manage_areas', [], session('locale')) }}
            </h2>
            <button @click="open = true"
                ...
                <span>{{ trans('messages.add_new_area', [], session('locale')) }}</span>
            </button>
        </div>
        ...
        <div class="w-full mt-6 mb-8">
            <div class="relative flex items-center bg-white/90 backdrop-blur-md rounded-2xl shadow-md border border-[var(--accent-color)] max-w-md mx-auto sm:mx-0 px-3 py-2 transition-all duration-300 hover:shadow-lg hover:bg-white">
              <input
                id="search_area"
                type="text"
                placeholder="{{ trans('messages.search_area', [], session('locale')) }}"
                ...
                <thead class="bg-gray-50 border-b border-[var(--border-color)]">
                    <tr>
                        <th class="px-4 sm:px-6 py-4 font-semibold text-[var(--text-secondary)]">
                            {{ trans('messages.area_name_en', [], session('locale')) }}
                        </th>
                        <th class="px-4 sm:px-6 py-4 font-semibold text-[var(--text-secondary)]">
                            {{ trans('messages.area_name_ar', [], session('locale')) }}
                        </th>
                        <th class="px-4 sm:px-6 py-4 font-semibold text-[var(--text-secondary)]">
    ...
    <div x-show="open" x-cloak 
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" id="add_area_modal" x-ref="areaModal">
        <div @click.away="open = false"
            class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 sm:p-8">
            <div class="flex justify-between items-start mb-6">
                <h1 class="text-xl sm:text-2xl font-bold">
                    <span x-text="edit ? '{{ trans('messages.edit_area', [], session('locale')) }}' : '{{ trans('messages.add_area', [], session('locale')) }}'"></span>
                </h1>
            ...
            <form id="area_form">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text"
                            placeholder="{{ trans('messages.area_name_en_placeholder', [], session('locale')) }}"
                            name="area_name_en" id="area_name_en"
                    ...
                    <div>
                        ...
                        <input type="text"
                            placeholder="{{ trans('messages.area_name_ar_placeholder', [], session('locale')) }}"
                            name="area_name_ar" id="area_name_ar"
                            class="w-full border rounded-lg p-3 focus:ring focus:ring-[var(--primary-color)]">
                    </div>
