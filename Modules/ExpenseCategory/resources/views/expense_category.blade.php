...
@section('main')
@push('title')
<title>{{ trans('messages.expense_category', [], session('locale')) }}</title>
@endpush
...
<main class="flex-1 p-8 bg-background-light dark:bg-background-dark overflow-y-auto" 
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
            <h2 class="text-2xl sm:text-4xl font-bold text-[var(--text-primary)]">
                {{ trans('messages.manage_expense_categories', [], session('locale')) }}
            </h2>
            <button @click="open = true"
                ...
                <span>{{ trans('messages.add_new_expense_category', [], session('locale')) }}</span>
            </button>
        </div>
        ...
        <div class="w-full mt-6 mb-8">
            <div class="relative flex items-center bg-white/90 backdrop-blur-md rounded-2xl shadow-md border border-[var(--accent-color)] max-w-md mx-auto sm:mx-0 px-3 py-2 transition-all duration-300 hover:shadow-lg hover:bg-white">
              <input
                id="search_expense_category"
                type="text"
                placeholder="{{ trans('messages.search_expense_category', [], session('locale')) }}"
    ...
    <div x-show="open" x-cloak 
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" id="add_expense_category_modal" x-ref="expenseCategoryModal">
        <div @click.away="open = false"
            class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 sm:p-8">
            <div class="flex justify-between items-start mb-6">
                <h1 class="text-xl sm:text-2xl font-bold">
                    <span x-text="edit ? '{{ trans('messages.edit_expense_category', [], session('locale')) }}' : '{{ trans('messages.add_expense_category', [], session('locale')) }}'"></span>
                </h1>
                <button @click="open = false" class="text-gray-400 hover:text-gray-600" id="close_modal">
                    <span class="material-symbols-outlined text-3xl">close</span>
                </button>
            </div>
            <form id="expense_category_form">
                @csrf
                <div class="space-y-6">
                    <div>
                        <label class="block text-base font-medium mb-2">
                            {{ trans('messages.category_name', [], session('locale')) }} <span class="text-red-500">*</span>
                        </label>
                        <input type="text"
                            placeholder="{{ trans('messages.category_name_placeholder', [], session('locale')) }}"
                            name="category_name" id="expense_category_name" required
                            class="w-full border rounded-lg p-3 focus:ring focus:ring-[var(--primary-color)]">
                    </div>

                    <input type="hidden" id="expense_category_id" name="expense_category_id">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            name="notes" id="expense_category_notes" rows="4"
