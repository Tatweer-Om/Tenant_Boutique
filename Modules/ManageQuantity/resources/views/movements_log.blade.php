...
<main class="flex-1 p-4 md:p-6">
  <div class="w-full max-w-screen-xl mx-auto space-y-6">
    ...
    <section class="bg-white border border-pink-100 rounded-2xl p-6 shadow-sm">
      <form method="GET" action="{{ route('movements_log') }}" class="flex flex-wrap items-center gap-3">
              ...
              @foreach($transfers as $transfer)
                <tr class="border-t hover:bg-pink-50/60">
                  ...
                  <td class="px-3 py-2 text-center">
                    <button type="button"
                            data-transfer="{{ htmlspecialchars(json_encode($transfer), ENT_QUOTES, 'UTF-8') }}"
                            onclick="openDetails(this.getAttribute('data-transfer'))"
