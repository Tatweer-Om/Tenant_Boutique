                ...
                <tr class="border-t hover:bg-pink-50/60">
                  ...
                  <td class="px-3 py-2 text-center">
                    <template x-if="h.attachment_path">
                      <a :href="(h.attachment_path ? '{{ url("/") }}/' + h.attachment_path : '#')" target="_blank" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <template x-if="settlementDetails.attachment_path">
                  <a :href="(settlementDetails.attachment_path ? '{{ url("/") }}/' + settlementDetails.attachment_path : '#')" target="_blank" 
...
function settlementPage(){
  return {
    ...
    async loadBoutiques(){
      try {
        const response = await fetch('{{ url("get_boutiques_list") }}');
    ...
    async loadTransfers(){
      ...
      try {
        ...
        const response = await fetch('{{ url("get_settlement_data") }}?' + params.toString());
        const data = await response.json();
    ...
    async openMovement(r){
      ...
      try {
        ...
        const response = await fetch('{{ url("get_settlement_transfer_details") }}?' + params.toString());
        const data = await response.json();
        if (response.ok && Array.isArray(data)) {
          this.movDetails = data.length === 0 ? [{id: 'no-data', date: '-', type: '{{ trans('messages.no_data_available', [], session('locale')) }}', from: '-', to: '-', qty: '-'}] : data;
        } else {
    ...
    async saveSettlement(){
      ...
      if (this.attachment.file) formData.append('attachment', this.attachment.file);
      try {
        const response = await fetch('{{ url("save_settlement") }}', {
          method: 'POST',
    ...
    async loadHistory(page = 1){
      ...
      try {
        ...
        const response = await fetch('{{ url("get_settlement_history") }}?' + params.toString());
        const result = await response.json();
    ...
    get historyFiltered(){ return this.history; },

    get paginationPages(){
      if (this.histLastPage <= 1) return [];
      if (this.histLastPage <= 10) return Array.from({length: this.histLastPage}, (_, i) => i + 1);
      let pages = [];
      ...
      if (endPage < this.histLastPage) { if (endPage < this.histLastPage - 1) pages.push('...'); pages.push(this.histLastPage); }
      return pages;
    },
    ...
    async openHistoryDetails(h){
      ...
      try {
        const params = new URLSearchParams({ settlement_code: h.no });
        const response = await fetch('{{ url("get_settlement_details") }}?' + params.toString());
        const data = await response.json();
    ...
    exportNewCSV(){
      ...
      const lines = [head];
      this.rows.forEach(r=> lines.push([r.code, r.color || '', r.size || '', r.sent, r.pulled, r.sellable, r.sold || 0, r.diff || 0, r.price || 0, r.total || 0]));
    ...
    exportHistoryCSV(){
      ...
      const lines = [head];
      this.historyFiltered.forEach(h=> lines.push([h.no, h.month, this.boutiqueName(h.boutique), h.items, h.amount, h.diff]));
    ...
    async init(){
      ...
      if (this.boutiques.length > 0) this.filters.boutique = this.boutiques[0].id;
    }
  }
}
</script>
