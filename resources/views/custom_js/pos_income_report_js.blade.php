<script>
    ...
    function loadReport(page = 1) {
        ...
        $.ajax({
            url: "{{ route('reports.pos_income.data') }}",
            type: "GET",
            data: { 
                from_date: fromDate,
                to_date: toDate,
            ...
            success: function(res) {
                if (res.success && res.orders && res.orders.length > 0) {
                    ...
                    res.orders.forEach(function(order) {
                        rows += `
                            <tr class="hover:bg-pink-50/50 transition-colors border-b">
                                ...
                                <td class="px-4 py-4 text-[var(--text-primary)] text-center whitespace-nowrap">${formatNumber(order.discount)}</td>
                                <td class="px-4 py-4 text-[var(--text-primary)] text-center whitespace-nowrap">${formatNumber(order.profit)}</td>
                                <td class="px-4 py-4 text-[var(--text-primary)] whitespace-nowrap">${order.added_by}</td>
                    ...
                    if (res.totals) {
                        ...
                        $('#totalDiscount').text(formatNumber(res.totals.discount));
                        $('#totalProfit').text(formatNumber(res.totals.profit));
                        $('#summaryCards').show();
                    }
                    
                    // Render pagination - always show if there are multiple pages
                    if (res.last_page && res.last_page > 1) {
                        ...
                        // If total items exceed per_page, show pagination even if last_page is 1
    ...
    $(document).ready(function() {
        ...
        // Handle pagination click - use event delegation on document for dynamically rendered links
        ...
        $('#exportPdfBtn').on('click', function() {
            ...
            var url = "{{ route('reports.pos_income.export_pdf') }}";
        ...
        $('#exportExcelBtn').on('click', function() {
            ...
            var url = "{{ route('reports.pos_income.export_excel') }}";
            if (params) url += '?' + params;
