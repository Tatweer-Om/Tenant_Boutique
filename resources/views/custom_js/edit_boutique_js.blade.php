...
$(document).ready(function () {
    ...
    $('#add_boutique').on('submit', function (e) {
        ...
        // --- Ajax Submit ---
        $.ajax({
            url: "{{ route('update_boutique') }}",
            type: "POST",
            data: $(this).serialize(),

            success: function (res) {
                if (res.success) {
                    show_notification('success', '<?= trans("messages.boutique_updated_success", [], session("locale")) ?>' );
                    $('#boutique_name').val(res.boutique.boutique_name);
        $('#shelf_no').val(res.boutique.shelf_no);
        $('#monthly_rent').val(res.boutique.monthly_rent);
        $('#rent_date').val(res.boutique.rent_date);
        $('#status').val(res.boutique.status);
        $('#boutique_address').val(res.boutique.boutique_address);

        // Update hidden ID if returned
        $('#boutique_id').val(res.boutique.id);
                } else {
                    show_notification(
